...
JsKoansReporter.PLATITUDES = ["Pondering", "Contemplating", "Considering", "Meditating on", "Witnessing"]
JsKoansReporter.PLATITUDE_ACTIONS = ["gives you new meaning", "fills you with peace", "pets your kitten", "expanded your awareness", "frees Rick Grimes to find his family"]
...
JsKoansReporter.prototype.reportSpecResults = function(spec) {
  ...
  if ( !results.passed() ) {
    ...
    description = "'" + spec.description + "' is harshing your karma."

  } else {
      var i = Math.floor(Math.random()*JsKoansReporter.PLATITUDES.length)
      var plat = JsKoansReporter.PLATITUDES[i];
      var action = JsKoansReporter.PLATITUDE_ACTIONS[Math.floor(Math.random()*JsKoansReporter.PLATITUDE_ACTIONS.length)]
      description = plat + " '" + spec.description + "' " + action
  }
